...
from models import LSTM, ASCNN, ASGCN,CGCN
...
class Instructor:
    def __init__(self, opt):
        ...
        self.word_embedding_embedding_matrix = absa_dataset.embedding_matrix
        self.model = opt.model_class(self.word_embedding_embedding_matrix, opt).to(opt.device)
        self._print_args()
        self.global_f1 = 0.
    ...
    def _reset_params(self):
        for p in self.model.parameters():
            ...
            if p.size()[0] == self.word_embedding_embedding_matrix.shape[0]:
                print("found embedding and do not want to random inilization")
                pass
            if p.requires_grad:
    ...
    def run(self, repeats=1):
        ...
        if not os.path.exists('log/'):
            os.mkdir('log/')
        params ="_".join([ str(para) for para in [opt.complex,opt.concat,opt.schema,opt.pe]  ])
        print(params)
        f_out = open('log/'+self.opt.model_name+'_'+self.opt.dataset+"_"+params+'_val.txt', 'w', encoding='utf-8')
        f_out.write(params+"\n")
        max_test_acc_avg = 0
        max_test_f1_avg = 0
        ...
        f_out.write('mean max_test_acc: {0}, mean max_test_f1: {1}\n'.format(max_test_acc_avg / repeats,max_test_f1_avg / repeats))
        f_out.close()

# python train.py --model_name cgcn --dataset rest14 --save True

if __name__ == '__main__':
    # Hyper Parameters
    parser = argparse.ArgumentParser()
    parser.add_argument('--model_name', default='cgcn', type=str)
    parser.add_argument('--dataset', default='sst2', type=str, help='cr,mr,mpqa,subj,sst2,TREC')
    ...
    parser.add_argument('--schema', default=0, type=int)
    parser.add_argument('--pe', default=1, type=int)
    ...
    input_colses = {
        'lstm': ['text_indices'],
        'cgcn': ['text_indices', 'dependency_graph'],
        'ascnn': ['text_indices', 'aspect_indices', 'left_indices'],
        'asgcn': ['text_indices', 'dependency_graph'],
